...
%       Demodulation
instantaneous_phase = unwrap(angle(hilbert(mod_message))); 
instantaneous_freq = diff([instantaneous_phase(1), instantaneous_phase]) * f_sample / (2*pi);
demodulated_freq = (instantaneous_freq - f_carrier)/k_freq;

f_cutoff = max(f_message)*2;
demod_message = lowpass(demodulated_freq, f_cutoff, f_sample);
